...
func WeeklySummarySystem(periodType, lang string) string {
	if lang == "en" {
		...
		return fmt.Sprintf("You are a personal growth assistant helping users review %s work and learning, providing in-depth analysis and constructive feedback. Response must be pure JSON.", periodScope)
	}
...
Return in JSON format (no markdown code blocks):
{
  ...
  "suggestions": "%s suggestions (provide 3-7 actionable suggestions; if data is sparse, explain why and suggest data/process improvements)",
